...
def build_base_command(cf):
    ...
    """
    cmd = ffmpeg
    cmd = add_input(cmd, cf)
    cmd = add_preset(cmd, cf)
    cmd = add_key_min(cmd, cf)
    cmd = add_gop_size(cmd, cf)
    cmd = add_threshold(cmd, cf)
    cmd = add_fps(cmd, cf)
    cmd = add_video_codec(cmd, cf)
    cmd = add_pixel_format(cmd, cf)
    cmd = add_audio_codec(cmd, cf)
    cmd = add_audio_bitrate(cmd, cf)
    cmd = add_audio_channel(cmd, cf)
    cmd = add_audio_rate(cmd, cf)
    cmd = add_filters(cmd, cf)
    cmd = add_video_mapping(cmd, cf)
    cmd = add_audio_mapping(cmd, cf)
    return cmd
...
def build_dash_command(cf):
    ...
    cmd = add_dash_attributes(cmd, cf)
    return cmd
