...
from apps.video_transcoding_off import models, strategy, defaults
from apps.video_transcoding_off.celery import app
from apps.video_transcoding_off.transcoding import profiles
...
class TranscodeVideo(LoggerMixin, celery.Task):
    ...
    def select_for_update(self, video_id: int, status: int) -> models.Video:
        ...
        """
        try:
            video = Video.objects.select_for_update(skip_locked=True, of=("self",)).get(
                pk=video_id
            )
        ...
        if video.task_id != UUID(self.request.id):
            self.logger.error("Unexpected video %s task_id %s", video.id, video.task_id)
            raise ValueError(video.task_id)

        if video.status != status:
            self.logger.error(
                "Unexpected video %s status %s", video.id, video.get_status_display()
            )
    ...
    def unlock_video(
        ...
        try:
            ...
            raise RuntimeError("Can't unlock locked video %s: %s", video_id, repr(e))

        video.change_status(status, error=error, metadata=meta, duration=duration)
    ...
    @staticmethod
    def init_strategy(
        source_uri: str, basename: str, preset: profiles.Preset
    ) -> strategy.Strategy:
...
transcode_video: TranscodeVideo = app.register_task(TranscodeVideo())  # type: ignore
