...
from apps.video_transcoding_off.tests.base import BaseTestCase
from apps.video_transcoding_off.transcoding import transcoder
from apps.video_transcoding_off.transcoding.profiles import DEFAULT_PRESET
...
class TranscodingTestCase(MediaInfoMixin, BaseTestCase):
    ...
    def setUp(self):
        ...
        self.transcoder = transcoder.Transcoder(self.source, self.dest, DEFAULT_PRESET)

        self.runner_mock = mock.MagicMock(return_value=(0, "", ""))

        self.runner_patcher = mock.patch(
            "fffw.encoding.ffmpeg.FFMPEG.runner_class", return_value=self.runner_mock
        )
