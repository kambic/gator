...
from apps.video_transcoding_off.transcoding import (
    transcoder,
    profiles,
    inputs,
    codecs,
...
class TranscoderTestCase(ProcessorBaseTestCase):
    ...
    def test_call(self):
        with mock.patch.object(
            self.transcoder, "process", return_value=mock.sentinel.rv
        ) as m:
    ...
    def test_process(self):
        with (
            mock.patch.object(
                self.transcoder, "prepare_ffmpeg", return_value=mock.sentinel.ff
            ) as prepare_ffmpeg,
            mock.patch.object(self.transcoder, "run") as run,
            mock.patch.object(
                self.transcoder, "get_result_metadata", return_value=mock.sentinel.rv
            ) as get_result_metadata,
        ):
    ...
    def test_run(self):
        ...
        with self.assertRaises(RuntimeError) as ctx:
            self.transcoder.run(ff)
        self.assertEqual(ctx.exception.args[0], "invalid ffmpeg return code 2")

    def test_prepare_ffmpeg(self):
        with (
            mock.patch.object(
                self.transcoder, "prepare_input", return_value=mock.sentinel.source
            ) as prepare_input,
            mock.patch.object(
                self.transcoder,
                "prepare_video_codecs",
                return_value=mock.sentinel.video_codecs,
            ) as prepare_video_codecs,
            mock.patch.object(
                self.transcoder, "prepare_output", return_value=mock.sentinel.dst
            ) as prepare_output,
            mock.patch.object(
                self.transcoder,
                "scale_and_encode",
                return_value=mock.Mock(ffmpeg=mock.sentinel.ffmpeg),
            ) as scale_and_encode,
        ):
    ...
    def test_scale_and_encode(self):
        ...
        source = inputs.Input(
            streams=(
                Stream(VIDEO, meta=self.meta.video),
...
class SplitterTestCase(ProcessorBaseTestCase):
    ...
    def test_prepare_ffmpeg(self):
        ...
        expected = [
            ...
            "mkv",
            "-segment_list",
            "/dst/source-video.m3u8",
            "-segment_list_type",
            "m3u8",
            "-segment_time",
            defaults.VIDEO_CHUNK_DURATION,
...
class SegmentorTestCase(ProcessorBaseTestCase):
    ...
    def test_prepare_ffmpeg(self):
        ...
        expected = [
            ...
            "-y",
            "-i",
            "/results/source-video.m3u8",
            "-i",
            "/sources/source-audio.mkv",
            "-map",
            "0:v:0",
            "-c:v:0",
            "copy",
