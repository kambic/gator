...
from apps.video_transcoding_off.transcoding import extract
...
class SourceExtractorTestCase(ExtractorBaseTestCase):
    ...
    def test_mediainfo(self):
        try:
            self.mediainfo_pacher.stop()
            with mock.patch(
                "pymediainfo.MediaInfo.parse", return_value=mock.sentinel.mi
            ) as m:
...
class MKVVideoSegmentTestsMixin(MKVVideoSegmentTestsMixinTarget):
    ...
    def test_ffprobe(self):
        try:
            self.ffprobe_patcher.stop()
            pi = ffprobe.ProbeInfo(streams=[{}], format={})
            # noinspection PyTypeChecker
            content = json.dumps(asdict(pi))
            with mock.patch(
                "video_transcoding.transcoding.extract.FFProbe",
            ) as m:
...
class VideoSegmentExtractorTestCase(MKVVideoSegmentTestsMixin, ExtractorBaseTestCase):
    analyzer = "MKVSegmentAnalyzer"
    extractor_class = extract.VideoSegmentExtractor


class VideoResultExtractorTestCase(MKVVideoSegmentTestsMixin, ExtractorBaseTestCase):
...
class SplitExtractorTestCase(ExtractorBaseTestCase):
    ...
    @staticmethod
    def extractor_class():
        return extract.SplitExtractor(
            video_playlist="source-video.m3u8", audio_file="source-audio.mkv"
        )
