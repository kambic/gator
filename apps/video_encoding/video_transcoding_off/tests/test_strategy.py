...
from apps.video_transcoding_off import strategy, defaults
from apps.video_transcoding_off.tests import base
from apps.video_transcoding_off.transcoding import profiles, workspace


class ResumableStrategyTestCase(base.ProfileMixin, base.MetadataMixin, TestCase):
    def setUp(self):
        super().setUp()
        ...
        self.strategy = strategy.ResumableStrategy(
            source_uri=source_uri, basename=basename, preset=profiles.DEFAULT_PRESET
        )
    ...
    def test_main_flow(self):
        with (
            mock.patch.object(
                self.strategy, "process", return_value=mock.sentinel.rv
            ) as p,
    ...
    def test_error_flow(self):
        with (
            mock.patch.object(self.strategy, "process", side_effect=Exception()) as p,
            mock.patch.object(self.strategy, "initialize") as i,
            mock.patch.object(self.strategy, "cleanup") as c,
        ):
    ...
    def test_initialize(self):
        self.strategy.initialize()
        expected = {"tmp-basename": {"sources": {}, "results": {}}}
        self.assertEqual(self.tmp_ws.tree, expected)
        expected = {"dst-basename": {}}
        self.assertEqual(self.dst_ws.tree, expected)

    def test_cleanup(self):
        self.tmp_ws.tree = {"tmp-basename": {"sources": {}, "results": {}}}
        self.dst_ws.tree = {"dst-basename": {}}
    ...
    def test_process(self):
        with (
            mock.patch.object(
                self.strategy, "analyze_source", return_value=mock.sentinel.src_rv
            ) as analyze_source,
            mock.patch.object(
                self.strategy, "select_profile", return_value=mock.sentinel.profile_rv
            ) as select_profile,
            mock.patch.object(self.strategy, "split") as split,
            mock.patch.object(
                self.strategy, "get_segment_list", return_value=["s1", "s2"]
            ) as get_segment_list,
            mock.patch.object(
                self.strategy,
                "process_segment",
                side_effect=[mock.sentinel.s1_rv, mock.sentinel.s2_rv],
            ) as process_segment,
            mock.patch.object(
                self.strategy,
                "merge_metadata",
                side_effect=[mock.sentinel.m1_rv, mock.sentinel.m2_rv],
            ) as merge_metadata,
            mock.patch.object(
                self.strategy, "merge", return_value=mock.sentinel.merge_rv
            ) as merge,
        ):
    ...
    def test_analyze_source_missing(self):
        ...
        with mock.patch.object(
            self.strategy, "_analyze_source", return_value=expected
        ) as m:
            meta = self.strategy.analyze_source()
    ...
    def test_select_profile_missing(self):
        ...
        content = json.dumps(asdict(expected))
        with mock.patch.object(
            self.strategy, "_select_profile", return_value=expected
        ) as m:
            profile = self.strategy.select_profile(src)
    ...
    def test_select_profile_call(self):
        src = self.make_meta(30.0)
        with mock.patch.object(
            self.strategy.preset, "select_profile", return_value=mock.sentinel.rv
        ) as m:
    ...
    def test_split_missing(self):
        ...
        # noinspection PyTypeChecker
        content = json.dumps(asdict(split))
        with mock.patch.object(self.strategy, "_split", return_value=split) as m:
            result = self.strategy.split(src)
        m.assert_called_once_with(src)
        self.assertEqual(result, split)
    ...
    def test_process_segment_missing(self):
        ...
        content = json.dumps(asdict(meta))
        with mock.patch.object(
            self.strategy, "_process_segment", return_value=meta
        ) as m:
            result = self.strategy.process_segment("s1")
        self.assertEqual(result, meta)
    ...
    def test_process_segment_call(self):
        ...
        with (
            mock.patch.object(self.strategy, "get_segment_meta", return_value=src) as m,
    ...
    def test_merge_call(self):
        ...
        with (
            mock.patch.object(
                self.strategy,
                "write_concat_file",
                return_value="memory:tmp-basename/results/concat.ffconcat",
            ) as m,
            mock.patch(target, autospec=True) as t,
        ):
            t.return_value.return_value = dst
