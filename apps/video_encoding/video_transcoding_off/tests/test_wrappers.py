...
from apps.video_transcoding_off.transcoding import inputs
...
class FFProbeWrapperTestCase(TestCase):
    def test_handle_stdout(self):
        ...
        with mock.patch.object(
            f, "logger", new_callable=mock.PropertyMock(return_value=m)
        ):
...
class InputsTestCase(TestCase):
    def test_init_inputs(self):
        src = inputs.input_file(
            "filename", Stream(kind=VIDEO), allowed_extensions="m3u8"
        )
