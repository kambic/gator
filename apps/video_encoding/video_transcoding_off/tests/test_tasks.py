...
from apps.video_transcoding_off import models, tasks
...
class TranscodeTaskVideoStateTestCase(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.video = models.Video.objects.create(
            status=models.Video.QUEUED, task_id=uuid4(), source="ftp://ya.ru/1.mp4"
        )
        cls = "video_transcoding.tasks.TranscodeVideo"
        self.handle_patcher = mock.patch(
            f"{cls}.process_video", return_value={"duration": 42.0}
        )
        self.handle_mock: mock.MagicMock = self.handle_patcher.start()
        self.retry_patcher = mock.patch(f"{cls}.retry", side_effect=Retry)
    ...
    def run_task(self):
        result = tasks.transcode_video.apply(
            task_id=str(self.video.task_id), args=(self.video.id,), throw=True
        )
        return result
...
class ProcessVideoTestCase(base.MetadataMixin, base.BaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.video = models.Video.objects.create(
            status=models.Video.PROCESS, source="ftp://ya.ru/1.mp4"
        )
