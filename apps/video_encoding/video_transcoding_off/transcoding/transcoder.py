...
from apps.video_transcoding_off import defaults
from apps.video_transcoding_off.transcoding import codecs, inputs, outputs, extract
from apps.video_transcoding_off.transcoding.metadata import Metadata
from apps.video_transcoding_off.transcoding.profiles import Profile
from apps.video_transcoding_off.utils import LoggerMixin
...
class Processor(LoggerMixin, abc.ABC):
    ...
    def __init__(self, src: str, dst: str, *, profile: Profile, meta: Metadata) -> None:
        super().__init__()
        self.src = src
        self.dst = dst
    ...
    @abc.abstractmethod
    def prepare_ffmpeg(self, src: Metadata) -> encoding.FFMPEG:  # pragma: no cover
        raise NotImplementedError
...
class Transcoder(Processor):
    ...
    def scale_and_encode(
        ...
        simd = SIMD(source, dst, overwrite=True, loglevel="repeat+level+info")
        # per-video-track scaling
        scaling_params = [(video.width, video.height) for video in self.profile.video]
...
class Splitter(Processor):
    ...
    def get_result_metadata(self, uri: str) -> Metadata:
        extractor = extract.SplitExtractor(
            video_playlist=self.source_video_playlist, audio_file=self.source_audio
        )
    ...
    def prepare_ffmpeg(self, src: Metadata) -> encoding.FFMPEG:
        ...
        ff = encoding.FFMPEG(input=source, loglevel="level+info", overwrite=True)
        ff > video_out
        ff > audio_out
        return ff

    def prepare_video_output(
        self, codecs_list: List[encoding.Codec]
    ) -> encoding.Output:
        return outputs.SegmentOutput(**self.get_video_output_kwargs(codecs_list))

    def prepare_audio_output(
        self, codecs_list: List[encoding.Codec]
    ) -> encoding.Output:
        return outputs.FileOutput(**self.get_audio_output_kwargs(codecs_list))

    def get_video_output_kwargs(
        self, codecs_list: List[encoding.Codec]
    ) -> Dict[str, Any]:
        return dict(
            codecs=codecs_list,
...
class Segmentor(Processor):
    ...
    def prepare_ffmpeg(self, src: Metadata) -> encoding.FFMPEG:
        ...
        video_codecs = [
            s > codecs.Copy(kind=VIDEO, bitrate=s.meta.bitrate)
        ...
        ff = encoding.FFMPEG(
            input=video_source, output=out, loglevel="level+info", overwrite=True
        )
    ...
    def prepare_output(self, codecs_list: List[encoding.Codec]) -> encoding.Output:
        return outputs.HLSOutput(**self.get_output_kwargs(codecs_list))
