...
from apps.video_transcoding_off.transcoding import analysis
from apps.video_transcoding_off.transcoding.ffprobe import FFProbe
...
class Extractor(LoggerMixin, abc.ABC):
    ...
    def ffprobe(
        self, uri: str, timeout: float = 60.0, **kwargs: Any
    ) -> ffprobe.ProbeInfo:
        self.logger.debug("Probing %s", uri)
        ff = FFProbe(
            uri, show_format=True, show_streams=True, output_format="json", **kwargs
        )
