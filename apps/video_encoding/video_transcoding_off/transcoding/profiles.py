...
class VideoCondition:
    ...
    def is_valid(self, meta: VideoMeta) -> bool:
        ...
        return (
            ...
            and meta.height >= self.min_height
            and meta.bitrate >= self.min_bitrate
            and meta.frame_rate >= self.min_frame_rate
            and (not self.min_dar or meta.dar >= self.min_dar)
            and (not self.max_dar or meta.dar <= self.max_dar)
        )
...
class AudioCondition:
    ...
    def is_valid(self, meta: AudioMeta) -> bool:
        ...
        return (
            ...
            and meta.sampling_rate >= self.min_sample_rate
        )
...
class Preset:
    ...
    def select_profile(self, video: VideoMeta, audio: AudioMeta) -> Profile:
        ...
        return Profile(
            ...
            container=Container(segment_duration=video_profile.segment_duration),
        )
...
DEFAULT_PRESET = Preset(
    ...
    audio_profiles=[
        AudioProfile(
            condition=AudioCondition(min_bitrate=0, min_sample_rate=0), audio=["192k"]
        ),
    ],
    video=[
        VideoTrack(
