...
def set_same_process_group(**kwargs: Any) -> None:
    ...
    os.setpgrp()
    logger.info("Set process group to %s for %s", os.getpgid(os.getpid()), os.getpid())
...
def send_term_to_children(**kwargs: Any) -> None:
    logger = get_logger(app.__module__)
    logger.warning("Received shutdown signal, sending SIGUSR1 to worker process group")
